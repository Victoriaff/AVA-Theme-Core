...
class cmb_Meta_Box_types {
	...
	public function render_repeatable_field() {
		...
		<p class="add-row">
			<a data-selector="<?php echo $table_id; ?>" class="add-row-button button" href="#"><?php esc_html_e( 'Add Row', 'engine-hosting' ); ?></a>
		</p>
	...
	protected function repeat_row( $class = 'repeat-row' ) {
		...
		<tr class="<?php echo $class; ?>">
			...
			<td class="remove-row">
				<a class="button remove-row-button" href="#"><?php esc_html_e( 'Remove', 'engine-hosting' ); ?></a>
			</td>
		</tr>
	...
	public function select( $args = array() ) {
		...
		//dd(sprintf( '<select%s>%s</select>%s', $attrs, $args['options'], $args['desc'] ) );
	...
	public function checkbox() {
		$meta_value = $this->field->escaped_value();
		$args = array( 'type' => 'checkbox', 'class' => 'cmb_option cmb_list', 'value' => '1', 'desc' => '' );
		if ( '1' == $meta_value ) {
 			$args['checked'] = 'checked';
 		}
		return sprintf(
			'<input type="hidden" name="%s" value="0">%s <label for="%s">%s</label>',
			$this->_name(),
			$this->input( $args ),
			$this->_id(),
			$this->_desc()
	...
	public function taxonomy_radio() {
		...
		if ( ! $terms ) {
			$options .= '<li><label>'. esc_html__( 'No terms', 'engine-hosting' ) .'</label></li>';
		} else {
			foreach ( $terms as $term ) {
				$args = array(
					'value' => $term->slug,
	...
	public function file_list() {
		...
		$this->input( array(
			...
			'value'  => esc_html__( 'Add or Upload File', 'engine-hosting' ),
			'name'  => '', 'id'  => '',
		) );
			...
			foreach ( $meta_value as $id => $fullurl ) {
				...
				if ( $this->is_valid_img_ext( $fullurl ) ) {
					...
					'<li class="img_status">',
						...
						'<p class="cmb_remove_wrapper"><a href="#" class="cmb_remove_file_button">'. esc_html__( 'Remove Image', 'engine-hosting' ) .'</a></p>
						'. $id_input .'
					</li>';
					...
					echo
					'<li>',
						esc_html__( 'File:', 'engine-hosting' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $fullurl, '" target="_blank" rel="external">'. esc_html__( 'Download', 'engine-hosting' ) .'</a> / <a href="#" class="cmb_remove_file_button">'. esc_html__( 'Remove', 'engine-hosting' ) .'</a>)
						'. $id_input .'
					</li>';
				}
			}
	...
	public function file() {
		...
		'<input class="cmb_upload_button button" type="button" value="'. esc_html__( 'Add or Upload File', 'engine-hosting' ) .'" />',
		$this->_desc( true );
		...
		'<div id="', $this->_id( '_status' ) ,'" class="cmb_media_status">';
			if ( ! empty( $meta_value ) ) {
				...
				if ( $this->is_valid_img_ext( $meta_value ) ) {
					...
					echo '<p class="cmb_remove_wrapper"><a href="#" class="cmb_remove_file_button" rel="', $cached_id, '">'. esc_html__( 'Remove Image', 'engine-hosting' ) .'</a></p>';
					echo '</div>';
				} else {
					...
					echo esc_html__( 'File:', 'engine-hosting' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $meta_value, '" target="_blank" rel="external">'. esc_html__( 'Download', 'engine-hosting' ) .'</a> / <a href="#" class="cmb_remove_file_button" rel="', $cached_id, '">'. esc_html__( 'Remove', 'engine-hosting' ) .'</a>)';
				}
			}
		echo '</div>';
	}
	...
	public function oembed() {
		...
		'<p class="cmb-spinner spinner" style="display:none;"><img src="'. esc_url(admin_url( '/images/wpspin_light.gif' )) .'" alt="spinner"/></p>',
