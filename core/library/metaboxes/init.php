...
class cmb_Meta_Box {
	...
	public static function autoload_helpers( $class_name ) {
		...
		$file = get_template_directory()."/core/library/metaboxes/helpers/$class_name.php";
		if ( file_exists( $file ) ) {
			@include( $file );
			return;
		}

		$file = plugin_dir_path( __FILE__ )."helpers/$class_name.php";
		if ( file_exists( $file ) ) {
			@include( $file );
			return;
		}
	...
	public function register_scripts() {
		...
		if ( 3.5 <= $wp_version ) {
			...
			if ( ! is_admin() ) {
				...
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => esc_html__( 'Clear', 'engine-hosting' ),
					'defaultString' => esc_html__( 'Default', 'engine-hosting' ),
					'pick'          => esc_html__( 'Select Color', 'engine-hosting' ),
					'current'       => esc_html__( 'Current Color', 'engine-hosting' ),
				) );
			}
		} else {
		...
		wp_localize_script( 'cmb-scripts', 'cmb_l10', apply_filters( 'cmb_localized_data', array(
			...
			'check_toggle'    => esc_html__( 'Select / Deselect All', 'engine-hosting' ),
		) ) );
	...
	public static function get_meta_box_url() {
		...
		$cmb_url = get_template_directory_uri().'/core/library/metaboxes';
...
function cmb_metabox_form( $meta_box, $object_id, $echo = true ) {
	...
	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, esc_html__( 'Save', 'engine-hosting') );
